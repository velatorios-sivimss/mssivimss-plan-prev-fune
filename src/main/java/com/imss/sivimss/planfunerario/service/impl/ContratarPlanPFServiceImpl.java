...
public class ContratarPlanPFServiceImpl implements ContratarPlanPFService {
        ...
        @Override
        public Response<?> agregarConvenioNuevoPF(DatosRequest request, Authentication authentication)
                        throws IOException {
                ...
                if (persona.getIdPersona() == null) {
                        queryPersona = convenioBean.generarQueryPersona(persona.getPersona(),
                                        usuarioDto.getIdUsuario().toString());
                }
                if (persona.getIdContratante() == null) {
                        queryContratante = convenioBean.generarQueryContratante(persona.getPersona(),
                                        usuarioDto.getIdUsuario().toString());
                }
                if (persona.getIdDomicilio() == null) {
                        queryDomicilio = convenioBean.generarQueryDomicilio(persona.getPersona().getCalle(),
                ...
                String queryConvenioPf = convenioBean.generarQueryConvenioPf(persona.getNombreVelatorio(),
                                persona.getIdPromotor(), persona.getIdVelatorio(), usuarioDto.getIdUsuario().toString(),
                                "1");
                String queryContratantePaquete = convenioBean.generarQueryContratantePaquete(persona,
                                usuarioDto.getIdUsuario().toString());
                ...
                for (int i = 0; i < persona.getPersona().getBeneficiarios().length; i++) {
                        ...
                        queryBeneficiario[i] = DatatypeConverter.printBase64Binary(
                                        convenioBean.generarQueryPersonaBeneficiaria(
                                                        persona.getPersona().getBeneficiarios()[i],
                                                        usuarioDto.getIdUsuario().toString()).getBytes("UTF-8"));
                        queryContratanteBeneficiarios[i] = DatatypeConverter.printBase64Binary(convenioBean
                                        .generarQueryContratanteBeneficiarios(
                                                        persona.getPersona().getBeneficiarios()[i].getParentesco(),
                                                        persona.getPersona().getClaveActa(),
                                                        usuarioDto.getIdUsuario().toString(),
                                                        persona.getPersona().getBeneficiarios()[i].getDocumentacion(),
                                                        authentication)
                                        .getBytes("UTF-8"));
                }
                ...
                mapa.put("datosPersonaContratante",
                                DatatypeConverter.printBase64Binary(queryPersona.getBytes("UTF-8")));
                ...
                mapa.put("datosContratantePaquete",
                                DatatypeConverter.printBase64Binary(queryContratantePaquete.getBytes("UTF-8")));
                mapa.put("datosBeneficiario", queryBeneficiario);
                mapa.put("datosContratanteBeneficiarios", queryContratanteBeneficiarios);
                mapa.put("datosValidacionDocumentos",
                                DatatypeConverter.printBase64Binary(convenioBean
                                                .generarQueryValidacionDocumentos(persona,
                                                                String.valueOf(usuarioDto.getIdUsuario()))
                                                .getBytes("UTF-8")));
                ...
                mapa.put("nombreContratante",
                                persona.getPersona().getNombre() + " " + persona.getPersona().getPrimerApellido()
                                                + " " + persona.getPersona().getSegundoApellido());
                ...
                return MensajeResponseUtil.mensajeResponse(
                                providerRestTemplate.consumirServicio(mapa, urlDominioConvenios + "/insertConvenios",
                                                authentication),
                                "30");
        }

        @Override
        public Response<?> agregarConvenioNuevoPFEmpresa(DatosRequest request, Authentication authentication)
                        throws IOException {

                String datosJson = String.valueOf(request.getDatos().get(AppConstantes.DATOS));
                UsuarioDto usuarioDto = json.fromJson((String) authentication.getPrincipal(), UsuarioDto.class);
                EmpresaAltaConvenio empresa = json.fromJson(datosJson, EmpresaAltaConvenio.class);
                String queryConvenioPf = convenioBean.generarQueryConvenioPf(empresa.getNombreVelatorio(),
                                empresa.getIdPromotor(), empresa.getIdVelatorio(), usuarioDto.getIdUsuario().toString(),
                                "0");
                String queryDomicilio = convenioBean.generarQueryDomicilio(empresa.getEmpresa().getCalle(),
                                ...
                                usuarioDto.getIdUsuario().toString());
                String queryEmpresaConvenioPF = convenioBean.generarQueryEmpresaConvenioPf(empresa.getEmpresa(),
                                usuarioDto.getIdUsuario().toString());
                ...
                mapa.put("datosEmpresaConvenio",
                                DatatypeConverter.printBase64Binary(queryEmpresaConvenioPF.getBytes("UTF-8")));
                ...
                return providerRestTemplate.consumirServicio(mapa, urlDominioConvenios + "/insertConvenios/empresa",
                                authentication);
        }

        @Override
        public Response<?> consultaPromotores(DatosRequest request, Authentication authentication) throws IOException {
                return providerRestTemplate.consumirServicio(convenioBean.consultarPromotores().getDatos(),
                                urlDominioConsulta,
                                authentication);
        }
        ...
        public Response<?> consultaPaquetes(DatosRequest request, Authentication authentication) throws IOException {
                ...
                return providerRestTemplate.consumirServicio(convenioBean.obtenerPaquetes(idVelatorio).getDatos(),
                                urlDominioConsulta, authentication);
        }

        @Override
        public Response<?> validaCurpRfc(DatosRequest request, Authentication authentication) throws IOException {
                try {
                        JsonObject objeto = (JsonObject) jsonParser
                                        .parse((String) request.getDatos().get(AppConstantes.DATOS));
                        RenapoResponse rp = new RenapoResponse();
                        String curp = String.valueOf(objeto.get("curp"));
                        String rfc = String.valueOf(objeto.get("rfc"));
                        Response responsePersona = providerRestTemplate.consumirServicio(
                                        convenioBean.consultarCurpRfc(curp, rfc).getDatos(), urlDominioConsulta,
                                        authentication);
                        if (responsePersona.getDatos().toString().equals("[]")) {
                                ...
                                Response respuestaRenapo = providerRestTemplate
                                                .consumirServicioExternoGet(urlRenapo + "/" + curp.replace("\"", ""));
                                JsonObject objRenapo = (JsonObject) jsonParser
                                                .parse(respuestaRenapo.getDatos().toString());
        ...
        public Response<?> consultaCP(DatosRequest request, Authentication authentication) throws IOException {
                ...
                return providerRestTemplate.consumirServicio(convenioBean.consultarCP(cp).getDatos(),
                                urlDominioConsulta,
                                authentication);
        }
        ...
        public Response<?> generarPDF(DatosRequest request, Authentication authentication) throws IOException {
                ...
                BusquedaInformacionReporteResponse datosComplementarios = buscarInformacionReporte(
                                pdfDto.getIdConvenio(),
                                authentication);
        ...
        @Override
        public Response<?> busquedaFolioPersona(DatosRequest request, Authentication authentication)
                        throws IOException {
                ...
                Response<?> responseContratante = providerRestTemplate.consumirServicio(
                                convenioBean.busquedaFolioPersona(folioConvenio).getDatos(), urlDominioConsulta,
                                authentication);
                if (!responseContratante.getDatos().toString().equals("[]")) {
                        contratanteResponse = Arrays
                                        .asList(modelMapper.map(responseContratante.getDatos(),
                                                        ContratanteResponse[].class));
                        beneficiariosResponse = Arrays
                                        .asList(modelMapper.map(
                                                        providerRestTemplate
                                                                        .consumirServicio(convenioBean
        ...
        @Override
        public Response<?> busquedaFolioEmpresa(DatosRequest request, Authentication authentication)
                        throws IOException {
                JsonObject objeto = (JsonObject) jsonParser.parse((String) request.getDatos().get(AppConstantes.DATOS));
                String folioConvenio = String.valueOf(objeto.get("folioConvenio"));
                return providerRestTemplate.consumirServicio(
                                convenioBean.busquedaFolioEmpresa(folioConvenio).getDatos(),
                                urlDominioConsulta, authentication);
        }
        ...
        public Response<?> busquedaRfcEmpresa(DatosRequest request, Authentication authentication) throws IOException {
                ...
                JsonObject objeto = (JsonObject) jsonParser.parse((String) request.getDatos().get(AppConstantes.DATOS));
                String rfc = String.valueOf(objeto.get("rfc"));
                return providerRestTemplate.consumirServicio(convenioBean.busquedaRfcEmpresa(rfc).getDatos(),
                                urlDominioConsulta, authentication);
        }

        @Override
        public Response<?> activarDesactivarConvenio(DatosRequest request, Authentication authentication)
                        throws IOException {
                JsonObject objeto = (JsonObject) jsonParser.parse((String) request.getDatos().get(AppConstantes.DATOS));
                UsuarioDto usuarioDto = json.fromJson((String) authentication.getPrincipal(), UsuarioDto.class);
                String folioConvenio = String.valueOf(objeto.get("folioConvenio"));
                ...
                switch (bandera) {
                        case "1":
                                ...
                                return providerRestTemplate.consumirServicio(
                                                convenioBean.cambiarEstatusConvenio("2", folioConvenio, usuarioDto)
                                                                .getDatos(),
                                                urlDominioActualizar, authentication);
                        case "0":
                                log.info("Desactivando convenio");
                                return providerRestTemplate.consumirServicio(
                                                convenioBean.cambiarEstatusConvenio("3", folioConvenio, usuarioDto)
                                                                .getDatos(),
                                                urlDominioActualizar, authentication);
                        default:
        ...
        public BusquedaInformacionReporteResponse buscarInformacionReporte(String folioConvenio,
                        Authentication authentication) throws IOException {
                BusquedaInformacionReporteResponse resultadoBusquedaInfo;
                Response<?> respuestaBusqueda = providerRestTemplate.consumirServicio(
                                convenioBean.busquedaFolioParaReporte(folioConvenio).getDatos(), urlDominioConsulta,
                                authentication);
                List<BusquedaInformacionReporteResponse> infoReporte = Arrays
                                .asList(modelMapper.map(respuestaBusqueda.getDatos(),
                                                BusquedaInformacionReporteResponse[].class));
